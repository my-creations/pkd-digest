---
title:
  en: 'Limitations of MELD in Isolated Polycystic Liver Disease: Fatal Traumatic Cyst Rupture Complicated by Abdominal Compartment Syndrome.'
  pt: 'Limitações do MELD na doença hepática poliquística isolada: rutura traumática fatal de quisto complicada por síndrome compartimental abdominal.'
date: 2026-01-01
issue: '2026-W33'
source:
  url: 'https://pubmed.ncbi.nlm.nih.gov/42609653/'
  name: 'PubMed | Case reports in hepatology'
tags: [research]
audience: [patients, clinicians]
summary:
  en: 'A man whose liver was crowded with cysts but whose blood tests looked fine scored too low for a liver transplant. After wasting away he was listed — then a bump burst a cyst and he died. MELD scores can miss real suffering in polycystic liver disease; repeat checks matter.'
  pt: 'Um homem com o fígado cheio de quistos mas análises boas teve pontuação baixa para transplante hepático. Depois de definhar foi listado — e uma pancada rompeu um quisto e matou-o. O MELD pode não ver o sofrimento real na doença hepática poliquística; repetir avaliações conta.'
clinicalNote:
  en: 'Single case report (isolated PLD). Illustrates MELD blind spots — mass effect, malnutrition, sarcopenia — and fatal traumatic rupture with compartment syndrome. Exception pathways exist: reassess longitudinally and counsel trauma risk in massive PLD.'
  pt: 'Relato de caso único (DHP isolada). Ilustra pontos cegos do MELD — efeito de massa, desnutrição, sarcopenia — e rutura traumática fatal com síndrome compartimental. Há vias de exceção: reavaliar ao longo do tempo e avisar do risco traumático na DHP volumosa.'
visitQuestion:
  en: 'If liver cysts affect my eating, weight or daily activities despite normal blood tests, how will you assess their impact?'
  pt: 'Se os quistos no fígado afetam a minha alimentação, peso ou atividades diárias apesar de análises normais, como será avaliado esse impacto?'
status: published
placeholder: false
---

<!-- DRAFT / NOT PUBLISHED — human review required; set issue + Dual Framing before publish -->
